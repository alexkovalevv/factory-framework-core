...
class Migrations {
	...
	public function migration_error_notice( $notices ) {
		...
		$notice_text .= "<br><br><a href='{$fix_migration_error_url}' class='button button-default'>" . __( 'I fixed, confirm migration', 'wbcr_factory_000' ) . "</a>";
		...
		$notices[] = array(
			...
			'dismissible'     => false,
			'dismiss_expires' => 0,
			'text'            => '<p><b>' . $this->plugin->getPluginTitle() . ' ' . __( 'migration error', 'wbcr_factory_000' ) . '</b><br>' . $notice_text . '</p>'
		);
	...
	public function debug_bar_notice( $notices ) {
		...
		$notice_text = __( "Plugin activated:", "wbcr_factory_000" ) . ' ' . date( "Y-m-d H:i:s", $this->plugin->getPopulateOption( 'plugin_activated', 0 ) ) . "<br>";
		$notice_text .= __( "Old plugin version (db):", "wbcr_factory_000" ) . ' ' . $this->plugin->getPopulateOption( 'plugin_version' ) . "<br>";
		$notice_text .= __( "Old plugin version (debug):", "wbcr_factory_000" ) . ' ' . $this->get_old_plugin_version() . "<br>";
		$notice_text .= __( "Current plugin version:", "wbcr_factory_000" ) . ' ' . $this->get_current_plugin_version() . "<br>";
		$notice_text .= __( "Need migration:", "wbcr_factory_000" ) . ' ' . ( $this->need_migration() ? "true" : "false" ) . "<br><br>";
		$notice_text .= "<a href='{$migrate_url}' class='button button-default'>" . __( "Migrate now", "wbcr_factory_000" ) . "</a><br>";
		...
		$notices[] = array(
			...
			'dismissible'     => false,
			'dismiss_expires' => 0,
			'text'            => '<p><b style="color:red;">' . $this->plugin->getPluginTitle() . ' ' . __( 'migrations DEBUG bar', 'wbcr_factory_000' ) . '</b><br>' . $notice_text . '</p>'
		);
	...
	private function get_classes( $path ) {
		...
		if ( ! function_exists( 'token_get_all' ) ) {
			throw new Exception( __( 'There is no PHP Tokenizer extension installed on your server, you cannot use the token_get_all function.', 'wbcr_factory_000' ) );
		}
